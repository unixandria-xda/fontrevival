...
alias curl='$MODDIR/tools/curl -kLsq --tr-encoding --tcp-fastopen --create-dirs --http2-prior-knowledge --retry 3 --retry-all-errors'
...
test_connection() {
  (curl -d "$P" "$U"/ping >/dev/null 2>&1) && return 0 || return 1
}
dl() {
